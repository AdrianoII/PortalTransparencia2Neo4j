...
licitacao = pd.read_csv(path_csv + '201908_Licitacao.csv',';',encoding='latin1', dtype = str,usecols = ['Número Licitação','Objeto','Situação Licitação','Valor Licitação','Data Resultado Compra','Nome Órgão','Código Órgão'])
...
#Como o conteudo deste csv poderia ser intreprertado como itens da licitação em vez da própria licitação
#Considero que licitação é definida apenas pelo seu numero
...
#Salvo os novos cv's
pessoa.to_csv(path_csv + 'Pessoa.csv')
empresa.to_csv(path_csv + 'Empresa.csv')
socioPessoa.to_csv(path_csv + 'SocioPessoa.csv')
socioEmpresa.to_csv(path_csv + 'SocioEmpresa.csv')
licitacao.to_csv(path_csv + 'Licitacao.csv')
contrato.to_csv(path_csv + 'Contrato.csv')
participante.to_csv(path_csv + 'Participou.csv')
...
arestas = [['Pessoa.csv','MATCH (o:Orgao {codigoOrgao: linha.COD_ORG_EXERCICIO}),(p:Pessoa {cpf : linha.CPF, nome : linha.NOME}) CALL apoc.create.relationship(p,"Servidor",{matricula : linha.MATRICULA, cargo : linha.DESCRICAO_CARGO, situacao : linha.SITUACAO_VINCULO},o) YIELD rel RETURN rel']
          ...
          ,['Participou.csv','MATCH (e:Empresa {cnpj: linha.CNPJ_Participante}),(l:Licitacao {numero : linha.Numero_Licitacao}) CALL apoc.merge.relationship(e,"Participou",{vencedor : linha.Flag_Vencedor},{vencedor : linha.Flag_Vencedor},l,{}) YIELD rel RETURN rel']
...
os.remove(path_csv + 'Pessoa.csv')
os.remove(path_csv + 'Empresa.csv')
os.remove(path_csv + 'SocioPessoa.csv')
os.remove(path_csv + 'SocioEmpresa.csv')
os.remove(path_csv + 'Licitacao.csv')
os.remove(path_csv + 'Contrato.csv')
os.remove(path_csv + 'Participou.csv')
